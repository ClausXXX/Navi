...
int ReadSettings(char *SettingsFileName, struct Settings *Settings)
{
	...
	while(!feof(SettingsFile))
	{
		...
		if(!strcmp(Settings->SettingName, "IonosphereMappingFunction"))
		{
			sscanf(CurrentString, "%*[^=]=%c", &Settings->IonoMapping);
		...
		if(!strcmp(Settings->SettingName, "IonosphereMappingParameters"))
		{
			sscanf(CurrentString, "%*[^=]=%f %f", &Settings->RE, &Settings->hI);
		}

		if(!strcmp(Settings->SettingName, "CorrectTroposphereDelay"))
		{
			sscanf(CurrentString, "%*[^=]=%c", &Settings->Troposphere);
			if(Settings->Troposphere == '\n' || Settings->Troposphere == '0')
			{
				Settings->Troposphere = 0;
            }
		}

		if(!strcmp(Settings->SettingName, "TroposphereMappingFunction"))
		{
			sscanf(CurrentString, "%*[^=]=%c", &Settings->TropoMapping);
...
int WriteSettings(char* SettingsFileName, struct Settings *Settings)
{
	...
	fprintf(SettingsFile, "IonosphereMappingFuntion=%c\n", Settings->IonoMapping);
	...
	fprintf(SettingsFile, "IonosphereMappingParameters=%f %f\n", Settings->RE,
																 Settings->hI);
	fprintf(SettingsFile, "CorrectTroposphereDelay=%c\n",
			Settings->Troposphere);
	fprintf(SettingsFile, "TroposphereMappingFunction=%c\n",
						...
						struct ComSolution *ComSolution)
{
    ...
    fprintf(outputfile, " IonosphereMappingFunction = %c\n", Settings->IonoMapping);
	...
	fprintf(outputfile, " IonosphereMappingParameters=%f %f\n", Settings->RE,
	                                                            Settings->hI);
	fprintf(outputfile, " CorrectTroposphereDelay = %c\n", Settings->Troposphere);
    fprintf(outputfile, " TroposphereMappingFunction = %c\n", Settings->TropoMapping);
