...
int main(int argc, char **argv)
{
	...
	int i, j, k, l, m, Cycles, Index1, Index2, DOY, NOfTayAbsTECFiles = 0,
		...
		IndexOfP1, IndexOfP2;
	register int Done;
	double tau, theta, deltat, TimeInSeconds, tec, f1, f2, ri, RA, Zd = 0.0,
		   Zw = 0.0,
	       x[3], y[3], z[3];
	clock_t t1, t2;
    ...
    printf("IonosphereMappingFunction = %c\n", Settings.IonoMapping);
	...
	printf("TECFile=%s\n", Settings.TECFile);
	printf("IonosphereMappingParameters=%f %f\n", Settings.RE, Settings.hI);
	printf("CorrectTroposphereDelay=%c\n", Settings.Trophosphere);
	printf("TroposphereMappingFunction = %c\n", Settings.TropoMapping);
...
//	RINEXNav->AlphaGPS[0] =  3.82E-08;
//	RINEXNav->AlphaGPS[1] =   1.49E-08;
//	RINEXNav->AlphaGPS[2] = - 1.79E-07;
//	RINEXNav->AlphaGPS[3] =   0.0;
//	RINEXNav->BetaGPS[0] =   1.43E+05;
//	RINEXNav->BetaGPS[1] =   0.0;
//	RINEXNav->BetaGPS[2] = - 3.28E+05;
//	RINEXNav->BetaGPS[3] =   1.13E+05;
////
//	TimeInSeconds = 20.0 * 3600.0 + 45.0 * 60.0;
//	do
//	{
//		delay = Klobuchar(40.0, 260.0, 210.0, 20.0, TimeInSeconds, RINEXNav->AlphaGPS, RINEXNav->BetaGPS) * 40.308E+4 / sqr(fL1_GPS);
//		printf("\n%5.2lf %lf", TimeInSeconds / 3600.0, delay);
//		TimeInSeconds += 3600.0;
//	}
//	while(TimeInSeconds < 86400.0);
//    TimeInSeconds = 20.0 * 3600.0 + 45.0 * 60.0;
//    do
//	{
//		delay = BDSKlobuchar(40.0, 260.0, 210.0, 20.0, TimeInSeconds, RINEXNav->AlphaGPS, RINEXNav->BetaGPS) * 40.308E+4 / sqr(fB1_BDS);
//		printf("\n\n%5.2lf %lf", TimeInSeconds / 3600.0, delay);
//		TimeInSeconds += 3600.0;
//	}
//	while(TimeInSeconds < 86400.0);
   ...
   //	printf("\n%lf %lf %lf\n", CurSolution[RINEXObs->CurrentEpoch].Q[0], CurSolution[RINEXObs->CurrentEpoch].Q[1], CurSolution[RINEXObs->CurrentEpoch].Q[2]);
	...
	if(Settings.Ionosphere == 'k')
	{
		if(Settings.Alpha[0] || Settings.Alpha[1] ||
		   ...
		   Settings.Beta[2] || Settings.Beta[3])
		{
			RINEXNav->AlphaBDSK[0] = Settings.Alpha[0];
			RINEXNav->AlphaBDSK[1] = Settings.Alpha[1];
			RINEXNav->AlphaBDSK[2] = Settings.Alpha[2];
			RINEXNav->AlphaBDSK[3] = Settings.Alpha[3];
			RINEXNav->BetaBDSK[0] = Settings.Beta[0];
			RINEXNav->BetaBDSK[1] = Settings.Beta[1];
			RINEXNav->BetaBDSK[2] = Settings.Beta[2];
			RINEXNav->BetaBDSK[3] = Settings.Beta[3];
		}
		else
		{
			if(!RINEXNav->AlphaBDSK[0] && !RINEXNav->AlphaBDSK[1] &&
			   !RINEXNav->AlphaBDSK[2] && !RINEXNav->AlphaBDSK[3] &&
			   !RINEXNav->BetaBDSK[0] && !RINEXNav->BetaBDSK[1] &&
			   !RINEXNav->BetaBDSK[2] && !RINEXNav->BetaBDSK[3])
		   {
				printf("There are no coefficients of the BDS Klobuchar ionosphere model");
				if(Settings.OutputType == TO_SCREEN || Settings.OutputType == TO_SCREEN_FILE)
				{
					getch();
				}
	...
	if(Settings.Ionosphere == 'n')
	{
		...
		else
		{
			if(!RINEXNav->AzGAL[0] && !RINEXNav->AzGAL[1] && !RINEXNav->AzGAL[2])
			{
				printf("There are no coefficients of the NTCM G ionosphere model");
				if(Settings.OutputType == TO_SCREEN || Settings.OutputType == TO_SCREEN_FILE)
				{
					getch();
				}
...
//	}
	...
	if(Settings.OutputType == TO_FILE || Settings.OutputType == TO_SCREEN_FILE)
	{
		   ...
		   strstr(Settings.FrequencyMode, "SP"))
		{
			...
			{
				strcpy(CurrentString, OutputFileParams);
				sprintf(OutputFileParams, "%s_I%c%c", CurrentString, Settings.Ionosphere,
				                                                     Settings.IonoMapping);
			}
		}
		...
		{
			strcpy(CurrentString, OutputFileParams);
			sprintf(OutputFileParams, "%s_T%c%c", CurrentString, Settings.Troposphere,
			                                                     Settings.TropoMapping);
		}
		  ...
		  Settings.FinalTime && RINEXObs->CurrentEpoch < RINEXObs->NOfEpochs)
	{
		...
		for(i = 0; i < RINEXObs->Epochs[RINEXObs->CurrentEpoch].NOfSattelites; i++)
		{
			...
			if(RINEXObs->Epochs[RINEXObs->CurrentEpoch].Sattelites[i][1] == 0)
			{
				IndexOfC1 = -1;
				IndexOfC2 = -1;
				IndexOfP1 = -1;
					...
					if(Sattelites[i].Valid)
					{
						if(Ephemeris == BOARD)
						{
							...
							if(Sattelites[i].Number[0] == 'R')
							{
                                ...
                                if(strstr(Settings.FrequencyMode, "SC2") || strstr(Settings.FrequencyMode, "SP2"))
								{
									Sattelites[i].dt -= Sattelites[i].dTauN;
								}
							}
...
//							}
						...
						DecartToGeo(CurSolution[RINEXObs->CurrentEpoch].Q[0],
									CurSolution[RINEXObs->CurrentEpoch].Q[1],
									CurSolution[RINEXObs->CurrentEpoch].Q[2],
									&CurSolution[RINEXObs->CurrentEpoch].B,
									&CurSolution[RINEXObs->CurrentEpoch].L,
									&CurSolution[RINEXObs->CurrentEpoch].H);
						if(Settings.Ionosphere)
						{
							RINEXObs->Epochs[RINEXObs->CurrentEpoch].TimeInHours =
							(float)RINEXObs->Epochs[RINEXObs->CurrentEpoch].Hours +
							(float)RINEXObs->Epochs[RINEXObs->CurrentEpoch].Minutes / 60.0 +
							RINEXObs->Epochs[RINEXObs->CurrentEpoch].Seconds / 3600.0;
							if(Settings.Ionosphere == 'K')
							{
								...
								if(!Settings.IonoMapping)
								{
									Sattelites[i].I *= KlobMapping(Sattelites[i].El);
								}
							}
							...
							if(Settings.Ionosphere == 'A')
							{
											     ...
											     &GLOIonData.hmax, &GLOIonData.A, &GLOIonData.Bbot, &GLOIonData.Btop))
								{
									...
									if(!Settings.IonoMapping)
									{
										Sattelites[i].I *= GLOMapping(CurSolution[RINEXObs->CurrentEpoch].B, CurSolution[RINEXObs->CurrentEpoch].L, Sattelites[i].B, Sattelites[i].L, Sattelites[i].H * 1.0E-3);
									}
								}
								else
								{
							...
							if(Settings.Ionosphere == 'N')
							{
								...
								{
									pdSatLLHdeg[0] = CurSolution[RINEXObs->CurrentEpoch].B;
									pdSatLLHdeg[1] = CurSolution[RINEXObs->CurrentEpoch].L;
									pdSatLLHdeg[2] = Sattelites[i].H;
                                }
							...
							if(Settings.Ionosphere == 'n')
							{
								...
								doy =  (real_T)DateToDOY(RINEXObs->Epochs[RINEXObs->CurrentEpoch].Year,
														 RINEXObs->Epochs[RINEXObs->CurrentEpoch].Month,
														 RINEXObs->Epochs[RINEXObs->CurrentEpoch].Day);
                            ...
                            if(Settings.Ionosphere == 'k')
							{
								TimeInSeconds = RINEXObs->Epochs[RINEXObs->CurrentEpoch].Hours * 3600 +
												RINEXObs->Epochs[RINEXObs->CurrentEpoch].Minutes * 60 +
												RINEXObs->Epochs[RINEXObs->CurrentEpoch].Seconds;
								Sattelites[i].I = BDSKlobuchar(CurSolution[RINEXObs->CurrentEpoch].B, CurSolution[RINEXObs->CurrentEpoch].L, Sattelites[i].Az, Sattelites[i].El, TimeInSeconds, RINEXNav->AlphaBDSK, RINEXNav->BetaBDSK);
								if(!Settings.IonoMapping)
								{
									Sattelites[i].I *= IonoMapping(Sattelites[i].El, RE_BDS, hI_BDSK);
								}
							}
							...
							if(Settings.Ionosphere == 'B')
							{
								...
								if(!Settings.IonoMapping)
								{
									Sattelites[i].I *= IonoMapping(Sattelites[i].El, RE_BDS, hI_BDGIM);
								}
							}
							...
							if(Settings.Ionosphere == 'G')
							{
								...
								if(!Settings.IonoMapping)
								{
									Sattelites[i].I *= IonoMapping(Sattelites[i].El, RE_WGS84, hI_GEMTEC);
								}
							}
...
//										}
							...
							if(Settings.IonoMapping &&
							   Settings.Ionosphere != 'N' &&
							   Settings.Ionosphere != 's')
							{
								if(Settings.IonoMapping == 'K')
								{
									Sattelites[i].I *=
									KlobMapping(Sattelites[i].El);
								}

								if(Settings.IonoMapping == 'A')
								{
                                    DecartToGeo(Sattelites[i].x, Sattelites[i].y, Sattelites[i].z, &Sattelites[i].B, &Sattelites[i].L, &Sattelites[i].H);
									Sattelites[i].I *=
								...
								if(Settings.IonoMapping == 'n')
								{
                                    Sattelites[i].I *=
									MSLMMapping(Sattelites[i].El, RE_WGS84, hI_NTCM_G);
								}
								...
								{
									Sattelites[i].I *=
									IonoMapping(Sattelites[i].El, RE_BDS, hI_BDSK);
								}

								if(Settings.IonoMapping == 'B')
								{
									Sattelites[i].I *=
									IonoMapping(Sattelites[i].El, RE_BDS, hI_BDGIM);
								}

								if(Settings.IonoMapping == 'G')
								{
									Sattelites[i].I *=
									IonoMapping(Sattelites[i].El, RE_WGS84, hI_GEMTEC);
								}

								if(Settings.IonoMapping == 'T')
								{
									Sattelites[i].I *=
									IonoMappingT(CurSolution[RINEXObs->CurrentEpoch].B, Sattelites[i].El, RE_WGS84, hI_T);
								}

								if(Settings.IonoMapping == 'I')
								{
                                    Sattelites[i].I *=
									IonoMapping(Sattelites[i].El, Settings.RE, Settings.hI);
                                }
							}
						...
						if(Settings.Troposphere)
						{
							if(!Settings.TropoMapping ||
								Settings.Troposphere == 'P' ||
								Settings.Troposphere == 'M' ||
							    Settings.Troposphere == 'S')
							{
                                if(Settings.Troposphere == 'P')
								{
									Sattelites[i].T = PrimitiveTrop(Sattelites[i].El);
								}

								if(Settings.Troposphere == 'M')
								{
									Sattelites[i].T = ModifiedPrimitiveTrop(Sattelites[i].El);
								}

								if(Settings.Troposphere == 'G')
								{
									Sattelites[i].T = GCAT(CurSolution[RINEXObs->CurrentEpoch].H * 1.0E-3, Sattelites[i].El);
								}

								if(Settings.Troposphere == 'H')
								{
									if(!Settings.p)
									{
								...
								if(Settings.Troposphere == 'B')
								{
									if(!Settings.p)
									{
										StandartAtmosphere(CurSolution[RINEXObs->CurrentEpoch].H, &Weather);
									}
									Sattelites[i].T = Black(RINEXObs->Epochs[RINEXObs->CurrentEpoch].Month, CurSolution[RINEXObs->CurrentEpoch].B, Sattelites[i].El, RA, &Weather);
								}

								if(Settings.Troposphere == 'S')
								{
									if(!Settings.p)
									{
										StandartAtmosphere(CurSolution[RINEXObs->CurrentEpoch].H, &Weather);
								...
								if(Settings.Troposphere == 'N')
								{
									DOY = DateToDOY(RINEXObs->Epochs[RINEXObs->CurrentEpoch].Year,
													RINEXObs->Epochs[RINEXObs->CurrentEpoch].Month,
													RINEXObs->Epochs[RINEXObs->CurrentEpoch].Day);
									if(!Settings.p)
									{
							...
							else
							{
								if(Settings.Troposphere == 'G')
								{
									GCATZ(CurSolution[RINEXObs->CurrentEpoch].H * 1.0E-3, Sattelites[i].El, &Zd, &Zw);
								}
								...
								if(Settings.Troposphere == 'B')
								{
									...
									BlackZ(RINEXObs->Epochs[RINEXObs->CurrentEpoch].Month, CurSolution[RINEXObs->CurrentEpoch].B, &Weather, &Zd, &Zw);
								}
								...
								if(Settings.Troposphere == 'D')
								{
									...
									SaastamoinenDavisZ(CurSolution[RINEXObs->CurrentEpoch].B, CurSolution[RINEXObs->CurrentEpoch].H * 1.0E-3, &Weather, &Zd, &Zw);
								}

								if(Settings.TropoMapping == 'P')
								{
                                    Sattelites[i].T = PrimitiveMapping(Zd, Zw, Sattelites[i].El);
								}

								if(Settings.TropoMapping == 'M')
								{
									 Sattelites[i].T = ModifiedPrimitiveMapping(Zd, Zw, Sattelites[i].El);
								}

								if(Settings.TropoMapping == 'G')
								{
									Sattelites[i].T = BlackEisnerMapping(Zd, Zw, Sattelites[i].El);
								}

								if(Settings.TropoMapping == 'H')
								{
									Sattelites[i].T = HopfieldMapping(Zd, Zw, Sattelites[i].El);
								}

								if(Settings.TropoMapping == 'B')
								{
									if(!Settings.p)
									{
										StandartAtmosphere(CurSolution[RINEXObs->CurrentEpoch].H, &Weather);
									}
									Sattelites[i].T = BlackMapping(Zd, Zw, Sattelites[i].El, RA, &Weather);
								}

								if(Settings.TropoMapping == 'C')
								{
									Sattelites[i].T = ChaoMapping(Zd, Zw, Sattelites[i].El);
								}

								if(Settings.TropoMapping == 'D')
								{
									if(!Settings.p)
									{
										StandartAtmosphere(CurSolution[RINEXObs->CurrentEpoch].H, &Weather);
									}
									Sattelites[i].T = CfA22Mapping(Zd, Zw, Sattelites[i].El, &Weather);
								}

								if(Settings.TropoMapping == 'I')
								{
									if(!Settings.p)
									{
										StandartAtmosphere(CurSolution[RINEXObs->CurrentEpoch].H, &Weather);
									}
									Sattelites[i].T = IfadisMapping(Zd, Zw, Sattelites[i].El, &Weather);
								}

								if(Settings.TropoMapping == 'N')
								{
									DOY = DateToDOY(RINEXObs->Epochs[RINEXObs->CurrentEpoch].Year,
													RINEXObs->Epochs[RINEXObs->CurrentEpoch].Month,
													RINEXObs->Epochs[RINEXObs->CurrentEpoch].Day);
									...
									Sattelites[i].T = NeilMapping(Zd, Zw, DOY, CurSolution[RINEXObs->CurrentEpoch].B, CurSolution[RINEXObs->CurrentEpoch].H * 1.0E-3, Sattelites[i].El, &Weather);
								}
							}
						}
						else
