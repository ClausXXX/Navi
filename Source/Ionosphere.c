...
// frequency GPS users. IEEE Trans. Aerosp. Electron. Syst. 23 (3), 325–331.
//
...
// Модель ионосферной задержки Клобучара для ГНСС BeiDou
// B - географическая широта расчётной точки, °
// L - географическая долгота расчётной точки, °
// Az - азимут спутника относительно расчётной точки, рад
// El - угол возвышения спутника над горизонтом, рад
// BDSTime - время проведения измерений UTC с начала суток, с
// *Alpha, *Beta - массивы коэффицентов модели на текущие сутки
// Возвращаемое значение - вертикальное значение полного электронного
//                         содержания, TECU (10^16 эл/м^(-2))
//
//     Zhang, Q., Liu, Z., Hu, Z., Zhou, J., Zhao, Q., 2022. A modified BDS
// Klobuchar model considering hourly estimated night-time delays. Gps
// Solut. 26 (2), 49. https://doi.org/10.1007/s10291-022-01236-0.
//
//     CSNO, 2018. BeiDou navigation satellite system signal in space interface
// control document–open service signal B3I (Version 1.0). China
// Satellite Navigation Office, February 2018.
//
double BDSKlobuchar(double B, double L, double Az, double El, double BDSTime,
                    double *Alpha, double *Beta)
{
	int i;
	double psi, lambdam, phim, t, A4, A2, x, Tiono;
	B = B * DegToRad;
	L = L * DegToRad;
	psi =  M_PI_2 - El - asin(RE_BDS / (RE_BDS + hI_BDSK) * cos(El));
    phim = asin(sin(B) * cos(psi) + cos(B) * sin(psi) * cos(Az));
	lambdam = L + asin(sin(psi) * sin(Az) / cos(phim));
	t = 4.32E+4 * lambdam / M_PI + BDSTime;
	t = (int)t % 86400;
	phim /= M_PI;
	A4 = Beta[0] + phim * (Beta[1] + phim * (Beta[2] + phim * Beta[3]));
	if(A4 >= 172800.0)
	{
		A4 = 172800.0;
	}
	...
	A2 = Alpha[0] + phim * (Alpha[1] + phim * (Alpha[2] + phim * Alpha[3]));
	if(A2 < 0.0)
	{
		A2 = 0.0;
	}
	...
	if(x < A4 / 4)
	{
		Tiono = 5.0E-9 + A2 * cos(2.0 * M_PI * x / A4);
	}
	else
	{
		Tiono = 5.0E-9;
	}
	return c * Tiono * sqr(fB1_BDS) / 40.308E+4;
}
...
//
//     ГЛОНАСС Интерфейсный контрольный документ. Общее описание системы
// с кодовым разделением сигналов. Редакция 1.0 [Электронный ресурс] /
// АО «Российские космические системы». – Москва. – 2016. – Режим доступа:
...
// содержания, TECU (10^16 эл/м^(-2))
//
//     The BeiDou global broadcast ionospheric delay correction model (BDGIM)
// and its preliminary performance evaluation results / Yuan Y.,
// Wang N., Li Z., Huo X. // NAVIGATION. – 2019. – Vol. 66. – P. 55– 69.
// – DOI: https://doi.org/10.1002/navi.292
//
...
//
//     Ivanov, V.B. Global empirical modelling of the total electron content
// of the ionosphere for satellite radio navigation systems / V.B. Ivanov, G.D.
// Gefan, O.A. Gorbachev // Journal of Atmospheric and Solar-Terrestrial Physics.
// – 2011. – Vol. – 73.  1703–1707
//
...
// Наиболее распространённая функция отображения (пересчёт вертикального значения полного электронного
...
double IonoMappingT(double B, double El, double RE, double hI)
{
	float alpha = 1.0;
    B = fabs(B);
	if(B < 15.0)
...
// Modified Single Layer Model mapping function (пересчёт вертикального значения полного электронного
// содержания в наклонное)
// El - угол возвышения спутника над горизонтом, рад
// Re - радиус Земли, км
// hi - высота ионосферного слоя, км
...
// Positioning System,” Geod.-Geophys. Arb. Schweiz, Vol. 59, 1999
//
double MSLMMapping(double El, double RE, double hI)
{
	return 1.0 / sqrt(1.0 - sqr(RE * sin(0.9782 * (M_PI_2 - El)) / (RE + hI)));
}
