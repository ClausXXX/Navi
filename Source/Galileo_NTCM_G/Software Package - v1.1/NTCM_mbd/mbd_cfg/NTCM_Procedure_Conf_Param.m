function cs = NTCM_Procedure_Conf_Param()
% MATLAB function for configuration set generated on 21-Apr-2022 22:45:34
...
cs.set_param('Name', 'Config_OBSW'); % Name
...
cs.set_param('StopTime', 'End_Time');   % Stop time
...
cs.set_param('FixedStep', 'Sample_Time');   % Fixed-step size (fundamental sample time)
...
cs.set_param('SignalLoggingName', 'logsout_new');   % Signal logging name
...
cs.set_param('BufferReuse', 'off');   % Reuse local block outputs
...
cs.set_param('LifeSpan', '2');   % Application lifespan (days)
...
cs.set_param('ProdBitPerLongLong', 64);   % Number of bits per long long
...
cs.set_param('ProdLargestAtomicInteger', 'LongLong');   % Largest atomic integer size
cs.set_param('ProdLargestAtomicFloat', 'Double');   % Largest atomic floating-point size
cs.set_param('ProdIntDivRoundTo', 'Zero');   % Production hardware signed integer division rounds to
cs.set_param('ProdEndianess', 'BigEndian');   % Byte ordering
...
cs.set_param('CustomDefine', 'CheckOn=1');   % Defines
...
cs.set_param('CodeProfilingInstrumentation', 'detailed');   % Measure function execution times
...
cs.set_param('GenerateWebview', 'on');   % Generate model Web view
...
cs.set_param('ObjectivePriorities', {'Execution efficiency','RAM efficiency','MISRA C:2012 guidelines'});   % Prioritized objectives
...
cs.set_param('CustomSymbolStrFcn', '$M$F');   % Subsystem methods
cs.set_param('CustomSymbolStrFcnArg', '$N$M');   % Subsystem method arguments
cs.set_param('CustomSymbolStrBlkIO', 'lOut_$N$M');   % Local block output variables
...
cs.set_param('SimulinkBlockComments', 'off');   % Simulink block comments
...
cs.set_param('ERTDataHdrFileTemplate', 'JRCert_code_template.cgt');   % Header file template
cs.set_param('ERTDataSrcFileTemplate', 'JRCert_code_template.cgt');   % Source file template
cs.set_param('RateTransitionBlockCode', 'Inline');   % Rate Transition block code
cs.set_param('ERTHdrFileBannerTemplate', 'JRCert_code_template.cgt');   % Header file template
cs.set_param('ERTSrcFileBannerTemplate', 'JRCert_code_template.cgt');   % Source file template
